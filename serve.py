...
def ensure_status():
    ...
    if "status" not in cols:
        con.execute("ALTER TABLE jobs ADD COLUMN status TEXT DEFAULT ''"); con.commit()


def rows(min_score, days, view):
    ensure_status()
    con = sqlite3.connect(DB)
    q = "SELECT url, first_seen, score, payload, tailored, status FROM jobs WHERE score>=? ORDER BY first_seen DESC, score DESC"
    out = []
    for url, seen, score, payload, tailored, status in con.execute(q, (min_score,)):
        status = status or ""
        if view == "open" and status in ("applied", "skip"):
            continue
        if view == "applied" and status != "applied":
            continue
        j = json.loads(payload); j["tailored"] = json.loads(tailored) if tailored else None; j["first_seen"] = seen; j["status"] = status
        out.append(j)
...
<div class="meta">{e(j['company'])} · {e(j['remote_from'])} · {e(j.get('contract',''))} · {e(str(s.get('salary') or 'salary not shown'))} · years req: {e(str(s.get('years_required') or '?'))} · flags: {e(', '.join(s.get('flags') or []) or 'none')} · seen {e(j['first_seen'])}</div></div></div>
<p>{e(s.get('reason',''))}</p>{body}
<p><form method="post" action="/status" style="display:inline"><input type="hidden" name="url" value="{e(j['url'])}">
<button name="status" value="applied" class="sm">Applied</button> <button name="status" value="skip" class="sm">Skip</button>
{'<button name="status" value="" class="sm">Reopen</button>' if j.get('status') else ''}</form></p></div>"""
...
def set_status():
    ensure_status()
    con = sqlite3.connect(DB)
    con.execute("UPDATE jobs SET status=? WHERE url=?", (request.form.get("status", ""), request.form["url"])); con.commit()
